...
WHERE rnr=1
ORDER BY STUDY_ID;
SELECT * FROM t_pid; #67412

#SELECT DISTINCT STUDY_ID FROM t_pid;
/*
SELECT cf.STUDY_ID, cf.CONSENT_ACCEPTED, ca.STUDY_ID, ca.CONSENT_ACCEPTED
FROM t_pid_cf cf LEFT OUTER JOIN t_pid_ca ca 
ON cf.STUDY_ID=ca.STUDY_ID
WHERE ca.STUDY_ID IS NULL;
*/
#SELECT * FROM t_pid WHERE CONSENT_ACCEPTED IS NULL;
